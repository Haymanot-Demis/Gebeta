        ...
        authenticate(req.body.username, req.body.password, (err, result) => {
          ...
          user.save()
            .then(user => {
                ...
                return res.json({"success" : true, "status" : "Successfully Resgistered", user});
            })
            .catch(err =>{
                next(err)
            })
...
userRouter.route('/account/changePassword')
.post(authenticate.isAuthenticated, (req, res, next) => {
    ...
    .then(user => {
        ...
        console.log(user);
        user.changePassword(req.body.oldPassword, req.body.newPassword, (err, result) => {
            if (err) {
                res.statusCode = 400;
                res.contentType = 'application/json';
                res.json(err)
...
userRouter.route('/account/resetPassword')
.post((req, res, next) => {
...
userRouter.route('/account/resetPasswordRequest')
.post((req, res, next) => {
    ...
    .then( async (user) => {
        ...
        const userToken = await Tokens.create({user_id:user._id, token:hashedToken});
        ...
        transorter.sendMail({from:'haymedin21@gmail.com',to:user.email,subject:'Reset password',text:token}, (err, info) => {
            if (err){
