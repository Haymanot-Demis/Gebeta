...
const {isAuthenticated, verifyAdmin, verifyLoungeAdmin} = require('../authenticate/authenticate');
...
.put(isAuthenticated, async (req, res, next) => {
    ...
    const totalPrice = (req.body.quantity * dish.price)
    req.body.totalPrice = totalPrice
    Orders.findOneAndUpdate({user:req.user._id},{
        $set:{
            user:req.user._id, dish:req.params.id, ...req.body
        }
    },{
        new:true
    })
